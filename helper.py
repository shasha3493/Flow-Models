...
def load_smiley_face(n):
    ...
    # We divide n into three sets, two for eyes and one for smiley
    a = [[-1.5, 2.5]] + rand.randn(count // 3, 2) * 0.2 # 2d points sampled from nomarl distribution  with mean (-1.5, 2.5) and std = 0.2
    b = [[1.5, 2.5]] + rand.randn(count // 3, 2) * 0.2 # 2d points sampled from nomarl distribution  with mean (1.5, 2.5) and std = 0.2
    # smiley (np.c_ does concatenation columnwise)
    c = np.c_[2 * np.cos(np.linspace(0, np.pi, count // 3)),
              -np.sin(np.linspace(0, np.pi, count // 3))]
    c += rand.randn(*c.shape) * 0.2
    data_x = np.concatenate([a, b, c], axis=0)
    # add labels for points in different clusters as 0, 1, 2
    data_y = np.array([0] * len(a) + [1] * len(b) + [2] * len(c))
    perm = rand.permutation(len(data_x))
    # shuffle x, y and return 
...
def q1_save_results(dset_type, part, fn):
    ...
    save_training_plot(train_losses, test_losses, f'Dataset {dset_type} Train Plot',
                       f'results/{part}_dset{dset_type}_train_plot.png')
    show_2d_densities(densities, dset_type, fname=f'results/{part}_dset{dset_type}_densities.png')
    show_2d_latents(latents, train_labels, f'results/{part}_dset{dset_type}_latents.png')
...
def q2_save_results(fn):
    ...
    # since we trained the model by divinding the pixel values by 2, we multiply by 2 and clip from 0 to 1.9999 as none of the pixel values should be >=2
    samples = np.clip(samples.astype('float') * 2.0, 0, 1.9999)
    floored_samples = np.floor(samples) # we do floor and so all pixel values are either 0 or 1
